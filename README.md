...
Here is the how the engine works:
  ...
  -> The outcome you choose for the prediction will have a large impact on this engine. The 'BTC/ETH/SOL price at the end of 2026' prediction is essentially a cash-or-nothing option, so we can use the Black-Scholes formula for c/n option to calculate the theoritical price of this  outcome. The outcomes on this particular market are ranges (BTC between X and Y), so a bull call spread. To calculate the price of this outcome, we do price of X - price of Y.
  
 -> The most important part of this engine is determining the correlation between the perp and the prediction. We start by modeling their joint price behavior using a bivariate normal distribution. By applying Cholesky decomposition, we generate 90 days worth of correlated synthetic log returns for the crypto with its specific volatility. Then, we use the user's inputted spot price and work backwards to map the price from the synthetic returns. Using this 90-day synthetic price history, we calculate the theoretical price of the event contract on each previous day. We use the returns (not spot price) to find the correlation between the perp and prediction.
